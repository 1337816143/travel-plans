...
(function(){
  'use strict';
  const layers=window.TravelFloatingLayers,store=window.TravelStore;let guard=false,lastAssistantTrigger=null,bound=false;
  ...
  function syncStore(event){if(!store||!event)return;const patch={};if(event.name==='panel')patch.panelCollapsed=!event.open;if(event.name==='assistant')patch.assistantOpen=Boolean(event.open);if(event.name==='drawer')patch.routeDrawerState=event.meta?.state||window.TravelRouteDrawer?.state||'collapsed';if(Object.keys(patch).length)store.set(patch,{source:'floating-layer:'+event.name})}
  function coordinate(event){syncStore(event);if(guard||!mobile()||!event?.open)return;guard=true;try{
  ...
  function bind(){if(bound)return;bound=true;
    ...
    window.addEventListener('resize',scheduleMeasure,{passive:true});window.visualViewport?.addEventListener('resize',scheduleMeasure,{passive:true});layers?.sync?.();scheduleMeasure()
  }
  ...
  window.TravelLayoutCoordinator=Object.freeze({measure,scheduleMeasure,coordinate,syncStore,mobile,closePanel,closeAssistant,collapseDrawer,bind});
})();
