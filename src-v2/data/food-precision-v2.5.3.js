...
(function(){
  ...
  function markerHtml(point){
    ...
    return `<div class="wishlist-map-marker ${eat?'eat':'buy'} girlfriend-seven-marker ${check?'needs-check':'verified'}" role="button" tabindex="0" aria-label="女朋友必去${esc(point.category)}：${esc(point.name)}"><div class="girlfriend-seven-logo">${sevenSvg()}${check?'<span class="wishlist-check-dot">核</span>':''}<span class="girlfriend-seven-kind">${eat?'吃':'买'}</span></div><div class="wishlist-map-label"><b>${esc(label.replace(/^7·/,''))}</b><small>数字7专属 · ${check?'到店前核验':'位置已核验'}</small></div></div>`;
  }
