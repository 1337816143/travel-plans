/* v2.5.2 pure data selectors without DOM or map dependencies. */
(function(){
  'use strict';
  ...
  function visiblePointsPure(day=null,state=window.TravelStore?.state||{}){if(state.recommendationMode)return points().filter(point=>(typeof RECOMMENDED!=='undefined'?RECOMMENDED:[]).includes(point.id));return points().filter(point=>{if(window.TravelWishlistMap?.isPoint?.(point))return window.TravelWishlistMap.visible(point,day);if(day)return(point.days||[]).includes(day)||['酒店','住宿区域'].includes(point.category);return point.category!=='推荐'})}
