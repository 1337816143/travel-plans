(function(){
  ...
  let body;
  function finish(name,ok,error,result){metrics.calls++;metrics[ok?'success':'errors']++;const source=result?.source?` · ${result.source}`:'';metrics.last=`${new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})} · ${name}${ok?'成功':`失败：${error?.message||error||'未知错误'}`}${source}`;const op=metrics.operations[name]||(metrics.operations[name]={calls:0,success:0,errors:0,lastSource:'',lastReportedAt:''});op.calls++;op[ok?'success':'errors']++;op.lastSource=result?.source||op.lastSource;op.lastReportedAt=result?.reportedAt||op.lastReportedAt;render()}
  function assess(value){if(window.TravelServiceResult?.isResult?.(value))return{ok:value.ok,error:value.error?.message||value.error,result:value};return{ok:true,error:null,result:value}}
  function wrap(name,fn,setter){if(typeof fn!=='function')return;setter(function(...args){let result;try{result=fn.apply(this,args)}catch(error){finish(name,false,error);throw error}if(result&&typeof result.then==='function')return result.then(value=>{const assessed=assess(value);finish(name,assessed.ok,assessed.error,assessed.result);return value},error=>{finish(name,false,error);throw error});const assessed=assess(result);finish(name,assessed.ok,assessed.error,assessed.result);return result})}
  function snapshot(){return{calls:metrics.calls,success:metrics.success,failure:metrics.errors,errors:metrics.errors,last:metrics.last,operations:typeof structuredClone==='function'?structuredClone(metrics.operations):JSON.parse(JSON.stringify(metrics.operations))}}
  ...
  window.TravelServiceDiagnostics={metrics,snapshot,render,mount};
})();
