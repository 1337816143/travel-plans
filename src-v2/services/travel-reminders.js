(function(){
  ...
  function bookingItems(){
    const bindings=window.TravelDataCatalog?.bindings||{},items=[];
    for(const [name,value] of Object.entries(bindings))if(/BOOK|RESERV/i.test(name)&&Array.isArray(value))items.push(...value);
    return items;
  }
  function unresolvedBookingCount(){
    const nodes=[...document.querySelectorAll('[data-booking-progress]')];
    return nodes.filter(node=>{const v=String(node.value||node.dataset.value||'').toLowerCase();return!/(done|booked|complete|skip|abandon|已完成|已预约|已购|放弃)/.test(v)}).length;
  }
  function deadlineReminders(today){
    const out=[];
    for(const item of bookingItems()){
      const deadline=item.deadline||item.cutoff||item.bookingDeadline;if(!/^\d{4}-\d{2}-\d{2}$/.test(deadline||''))continue;
      const left=dayDiff(today,deadline);if(left<0||left>14)continue;
      out.push({id:`deadline-${item.id||item.name||deadline}`,tone:left<=2?'urgent':'warn',icon:'⏰',title:`${item.name||item.title||'预约事项'}${left===0?'今天截止':`剩余 ${left} 天`}`,detail:`页面中的明确截止日期为 ${deadline}，请以官方预约页最终规则为准。`});
    }
    return out;
  }
  function build(){
    ...
    for(const [date,cast] of Object.entries(current)){
      if(date<START||date>END)continue;const text=weatherText(cast),old=previous[date]||'';
      if(rain.test(text))out.push({id:`rain-${date}-${text}`,tone:'urgent',icon:'☔',title:`${date.slice(5).replace('-','月')}日存在降水天气`,detail:`高德预报：${text.replaceAll('|',' / ')}。公开天气不提供逐小时降雨概率，请临近当天继续刷新。`});
      else if(old&&old!==text)out.push({id:`weather-change-${date}-${text}`,tone:'warn',icon:'🔄',title:`${date.slice(5).replace('-','月')}日天气预报发生变化`,detail:`上次：${old.replaceAll('|',' / ')}；本次：${text.replaceAll('|',' / ')}。`});
    }
  ...
  function mount(){
    if(root)return root;const audit=document.getElementById('auditBox');if(!audit)return null;
    root=document.createElement('section');root.className='trip-reminder-center';root.id='tripReminderCenter';root.innerHTML='<div class="trip-reminder-head"><strong>出发提醒</strong><span class="trip-reminder-count" id="tripReminderCount">0</span></div><div class="trip-reminder-list" id="tripReminderList"></div><div class="trip-reminder-tools"><button type="button" id="prepareOfflineTrip">准备离线访问</button><button type="button" id="refreshTripReminders">刷新提醒</button></div>';
    audit.insertAdjacentElement('afterend',root);list=root.querySelector('#tripReminderList');countNode=root.querySelector('#tripReminderCount');root.querySelector('#refreshTripReminders').onclick=refresh;root.querySelector('#prepareOfflineTrip').onclick=prepareOffline;root.addEventListener('click',event=>{const button=event.target.closest('[data-reminder-dismiss]');if(button)dismiss(button.dataset.reminderDismiss)});return root;
  }
  ...
  window.TravelReminders={refresh,build,prepareOffline,get lastWeather(){return lastWeather}};
})();
