...
(function(){
  ...
  function routeEntries(day=schedule()){if(!day)return[];const seen=new Map();return routePoints(day).map((point,index)=>{const occurrence=seen.get(point.id)||0;seen.set(point.id,occurrence+1);return{point,index,occurrence,key:day.date+':'+occurrence+':'+point.id,segmentKey:index?day.date+':'+(index-1)+':'+routePoints(day)[index-1].id+':'+point.id:null}})}
  ...
  function wrapGlobals(){
    if(typeof filterDay==='function'&&!filterDay.__store){const original=filterDay;filterDay=Object.assign(function(day){const result=original(day);set({selectedDay:day,toolsDay:day,routeVisible:true,recommendationMode:false},{source:'filterDay'});return result},{__store:true})}
    if(typeof showAll==='function'&&!showAll.__store){const original=showAll;showAll=Object.assign(function(){const result=original();dispatch('SHOW_ALL',null,{source:'showAll'});return result},{__store:true})}
    if(typeof clearRoutes==='function'&&!clearRoutes.__store){const original=clearRoutes;clearRoutes=Object.assign(function(){const result=original();dispatch('CLEAR_ROUTES',null,{source:'clearRoutes'});return result},{__store:true})}
  ...
  window.TravelStore=Object.freeze({state,set,dispatch,subscribe,snapshot:clone,history:()=>history.slice(),selectors,wrapGlobals,get bridging(){return bridging}});
