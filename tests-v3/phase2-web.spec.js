...
test.beforeEach(async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: '独立打开 v2.5.7' })).toHaveAttribute(
    'href',
    '../index.html',
  );
...
test('serves the exact complete v2.5.7 guide beside the custom planner', async ({
  page,
  request,
}, testInfo) => {
  ...
  expect(stableHtml).toContain('<meta name="travel-map-version" content="2.5.7">');
  expect(stableHtml).toContain("candidates=['2.5.7','2.5.6','2.5.5','2.5.4','1.0.15']");
