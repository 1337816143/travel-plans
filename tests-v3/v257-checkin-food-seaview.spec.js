...
test('v2.5.7 and v3 ship the complete check-in data without fabricated search-only coordinates', async ({
  request,
}) => {
  ...
  expect(data.routes.map((route) => route.name)).toEqual(
  ...
  expect(sign.searchOnly).toBe(true);
  expect(sign.lat).toBeUndefined();
  expect(sign.lng).toBeUndefined();

  const wuren = data.locations.find((item) => item.id === 'wuren-shop');
  expect(wuren.priority).toBe(true);
  expect(wuren.searchOnly).toBe(true);

  const ohmo = data.locations.find((item) => item.id === 'ohmo-cafe');
  expect(ohmo.priority).toBe(true);
  expect(ohmo.cameras.length).toBeGreaterThanOrEqual(2);
});

test('check-in tab shows every requested spot, highlighted routes and multiple camera positions', async ({
  page,
}) => {
...
test('new photo routes and spots are integrated into the corresponding daily itinerary cards', async ({
  page,
}) => {
  const legacy = await openCheckin(page);
...
test('camera map stays mobile-safe and search-only locations do not create fake markers', async ({
  page,
}, testInfo) => {
