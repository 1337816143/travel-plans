...
test('v2.5.2 keeps corrected segments, GPX risk, pure services and mapped wishlist coherent',async({page},testInfo)=>{
  ...
  await page.route('**/*',async route=>{
    ...
    if(url.hostname==='api.open-meteo.com'&&url.pathname==='/v1/forecast'){
      ...
      for(let hour=0;hour<24;hour++){time.push(`2026-08-12T${String(hour).padStart(2,'0')}:00`);apparent_temperature.push(hour>=9&&hour<=17?34:27);precipitation_probability.push(hour===14?65:15);weather_code.push(hour===14?61:1)}
  ...
  const result=await page.evaluate(async()=>{
    ...
    const gpx=`<?xml version="1.0"?><gpx version="1.1"><trk><name>测试轨迹</name><trkseg><trkpt lat="36.0580" lon="120.3200"><ele>10</ele></trkpt><trkpt lat="36.0590" lon="120.3210"><ele>18</ele></trkpt><trkpt lat="36.0615" lon="120.3240"><ele>32</ele></trkpt></trkseg></trk></gpx>`;
    const parsed=TravelTrackStore.parseGpx(gpx);await TravelTrackStore.save('08-12',parsed.points,{name:'测试轨迹.gpx',source:'GPX'});
    ...
    TravelFinance.add({kind:'budget',category:'交通',amount:120,label:'交通预算',date:'08-12'});TravelFinance.add({kind:'actual',category:'交通',amount:45,label:'实际车费',date:'08-12'});
    const calendarEvent=TravelCalendarExport.itineraryEvents().find(item=>item.id==='itinerary-08-12');
    const mapMode=TravelWishlistMap.setMode('all'),mapped=TravelSelectors.visiblePoints('08-12',{recommendationMode:false}).filter(point=>TravelWishlistMap.isPoint(point));
    return{parsed,measured,pureSearch,pureCompare,finance:TravelFinance.summary('08-12'),calendar:TravelCalendarExport.calendarIcs([calendarEvent],'测试','PUBLISH'),mapMode,mapped:mapped.map(point=>({id:point.id,category:point.category,precision:point.precision})),resultShape:Object.keys(TravelServiceResult.success({ok:true},{source:'test'})).sort()};
  });
  expect(result.parsed.points.length).toBe(3);
  ...
  expect(result.calendar).toContain('UID:itinerary-08-12@travel-plans.local');
  expect(result.mapMode).toBe('all');
  expect(result.mapped).toHaveLength(10);
  expect(result.mapped.some(item=>item.category==='必吃')).toBe(true);
  expect(result.mapped.some(item=>item.category==='必买')).toBe(true);
  expect(result.mapped.some(item=>item.precision==='anchor')).toBe(true);
