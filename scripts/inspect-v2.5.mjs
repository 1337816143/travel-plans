...
for(const removed of ['trusted-personal-recommendation-store-unverified','girlfriend-seven-marker','girlfriend-seven-svg','数字7专属'])if(initial.includes(removed)||lazy.includes(removed)||read('src-v2/data/food-precision-v2.5.3.js').includes(removed)||read('src-v2/ui/food-search-panel.js').includes(removed))failures.push(`removed number-seven identity still present: ${removed}`);
