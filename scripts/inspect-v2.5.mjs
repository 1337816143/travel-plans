...
const manifest=JSON.parse(read('assets/v2.5.1/manifest.json'));
const budget=JSON.parse(read('BUNDLE_BUDGET_v2.5.1.json'));
const schema=JSON.parse(read('DATA_SCHEMA_REPORT_v2.5.1.json'));
const required={
  ...
  wishlistData:['src-v2/data/generated/wishlist.js','const GIRLFRIEND_WISHLIST='],
  wishlistUi:['src-v2/ui/wishlist-panel.js','window.TravelGirlfriendWishlist'],
  lazyLoader:['src-v2/ui/trip-tools-loader.js','assets/v2.5.1/lazy-tools.js']
};
const failures=[];
...
const initial=read('src/v2.5.1.html'),lazy=read('assets/v2.5.1/lazy-tools.js');
if(initial.includes('window.TravelTripOperations='))failures.push('trip operations leaked into initial HTML');
for(const token of ['window.TravelAvailability','window.TravelAvailabilityController','window.TravelTrackStore','window.TravelFinance','window.TravelTripOperations','window.TravelGirlfriendWishlist'])if(!lazy.includes(token))failures.push(`lazy bundle missing ${token}`);
if(!initial.includes('const GIRLFRIEND_WISHLIST='))failures.push('wishlist canonical data missing from initial data layer');
if(schema.counts.wishlistAttractions!==17||schema.counts.wishlistFood!==12)failures.push(`wishlist counts incomplete: ${schema.counts.wishlistAttractions}/${schema.counts.wishlistFood}`);
...
const report={version:'2.5.1',legacyBytes:Buffer.byteLength(legacy),initialGzip:manifest.gzipBytes,lazyGzip:manifest.lazyGzipBytes,totalGzip:manifest.totalGzipBytes,initialDelta:manifest.initialDeltaBytes,totalDelta:manifest.totalDeltaBytes,observableStore:true,pureSelectors:true,controllerSeparatedServices:true,versionedLocalData:true,bookingCalendarSync:true,availabilityExpiryHours:12,wishlist:{attractions:schema.counts.wishlistAttractions,foodTasks:schema.counts.wishlistFood,completionState:true,dailyHints:true,schemaGuard:true},practicalTools:['next-stop','wishlist','segment-correction','availability','gpx','finance','undo','calendar-update','accessibility','health-check'],layoutCoordinator:true,lazyToolsEagerlyCached:false,failures};
