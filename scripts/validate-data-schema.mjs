...
const VERSION='2.5.4',VALIDATED_FOR='2026-07-31-v2.5.4';
...
function binding(name,file){const code=fs.readFileSync(path.join(DIR,file),'utf8'),sandbox={};sandbox.window=sandbox;vm.createContext(sandbox);const precision=name==='GIRLFRIEND_WISHLIST'?fs.readFileSync(path.join(ROOT,'src-v2/data/food-precision-v2.5.4.js'),'utf8'):'';vm.runInContext(`${code}\n${precision}\nthis.__value=${name};`,sandbox,{filename:file});return structuredClone(sandbox.__value)}
