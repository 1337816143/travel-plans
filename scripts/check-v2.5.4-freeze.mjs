...
if(sha(source)!==EXPECTED_HTML_SHA256)throw new Error('Frozen v2.5.4 source HTML changed.');
const manifest=JSON.parse(text('assets','v2.5.4','manifest.json'));
if(manifest.version!==VERSION||manifest.sha256!==EXPECTED_HTML_SHA256||manifest.lazySha256!==EXPECTED_LAZY_SHA256)throw new Error('Frozen v2.5.4 manifest identity changed.');
const chunks=['payload-0.b64','payload-1.b64','payload-2.b64','payload-3.b64'].map(name=>Buffer.from(text('assets','v2.5.4',name).replace(/\s+/g,''),'base64'));
...
else console.log(`v2.5.4 rollback artifact passed: ${ROLLBACK_BRANCH}@${BASELINE_COMMIT}, ${EXPECTED_HTML_SHA256}`);
