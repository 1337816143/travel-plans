...
const ROOT=process.cwd(),VERSION='1.0.15',FALLBACK='1.0.14';
...
const hash=crypto.createHash('sha256').update(html).digest('hex'),deployment=fs.readFileSync(path.join(ROOT,'DEPLOYMENT.md'),'utf8');if(!deployment.includes(hash))fail('Deployment hash mismatch');
...
const loader=fs.readFileSync(path.join(ROOT,'index.html'),'utf8'),loaderScript=loader.match(/<script>([\s\S]*?)<\/script>/i)?.[1];if(!loaderScript)fail('Loader script missing');new vm.Script(loaderScript,{filename:'index-loader.js'});if(!loader.includes(`['${VERSION}','${FALLBACK}']`))fail('Loader fallback order incorrect');if(!loader.includes(`?loader=${VERSION}`))fail('Loader cache bust missing');
...
console.log(`Validation OK: v${VERSION}, gzip=${gzip.length}, html=${html.length}, sha256=${hash}; traffic toggle preserves view`);
