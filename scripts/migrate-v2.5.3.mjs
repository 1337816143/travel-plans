...
patch('scripts/build-v2.mjs',text=>{
  ...
  if(!text.includes("'food-search-tab'"))text=text.replace("'organized-trip-tools'","'food-search-tab','point-search-food-sync','girlfriend-seven-logo','xiaomujia-precision','yunnan-store-honesty','organized-trip-tools'");
  text=text.replace('v${VERSION} 旅行工具与美食地图版','v${VERSION} 美食检索与精确门店版')
    .replace('重构旅行工具为清晰的分组折叠布局；必吃必买关联真实门店或真实核验点，并在高德与 OSM 中使用统一的专属标识。','修复旅行工具顶部布局；新增美食检索标签页、点位检索同步和数字7专属标识；小木家门店精确化，云南锅锅米线保持诚实待核。')
    .replace('旅行工具分组布局、必吃必买真实地图点与专属标识版。','美食检索、点位检索同步、数字7专属标识与门店精确核验版。');
  return text;
});
...
patch('scripts/validate-data-schema.mjs',text=>{
  ...
  if(!text.includes('小木家漳州二路总店和参鸡汤明确'))text=text.replace("'饮料版本与试喝要求保留'","'饮料版本与试喝要求保留','小木家漳州二路总店和参鸡汤明确','云南锅锅米线不误配外地门店','12项女朋友项目使用数字7专属身份'");
  return text;
});
