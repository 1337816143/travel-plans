...
patch('scripts/build-v2.mjs',text=>{
  ...
  text=text.replace('v${VERSION} 旅行工具与美食地图版','v${VERSION} 美食检索与精确门店版')
    .replace('重构旅行工具为清晰的分组折叠布局；必吃必买关联真实门店或真实核验点，并在高德与 OSM 中使用统一的专属标识。','修复旅行工具顶部布局；新增美食检索标签页和点位检索同步；保留原来的必吃／必买图案；小木家门店精确化，云南锅锅米线保持诚实待核。')
