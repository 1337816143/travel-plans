...
const VERSION='2.5.7';
const PREVIOUS='2.5.6';
const PREVIOUS2='2.5.5';
const ROLLBACK='2.5.4';
const STABLE='1.0.15';
const DATE='2026-08-09';
const RELEASE_TIMESTAMP='2026-08-09T16:57:00+08:00';
const FROZEN_V254_HASH='264fda8953fda2773cfe73f77372f20963ed0821acfa1701ac76bea872f2c027';

function run(script){const result=spawnSync(process.execPath,[script],{cwd:ROOT,stdio:'inherit'});if(result.error)throw result.error;if(result.status!==0)throw new Error(`${script} exited with ${result.status}`)}
...
function mustReplace(text,pattern,replacement,label){const next=text.replace(pattern,replacement);if(next===text)throw new Error(`v${VERSION} build token missing: ${label}`);return next}

run('scripts/build-v2.5.6-current.mjs');
const previousManifest=JSON.parse(read('assets',`v${PREVIOUS}`,'manifest.json'));
const previousHtml=read('src',`v${PREVIOUS}.html`);
if(sha(previousHtml)!==previousManifest.sha256)throw new Error(`v${PREVIOUS} source does not match its manifest`);
if(previousManifest.frozenRollbackSha256!==FROZEN_V254_HASH)throw new Error('v2.5.6 frozen rollback hash drifted');

const addonCss=read('src-v2.5.7','checkin-food-seaview.css');
const addonJs=read('src-v2.5.7','checkin-food-seaview.js');
const checkin=JSON.parse(read('data','qingdao','checkin','checkin-guide.v1.json'));
new vm.Script(addonJs,{filename:'src-v2.5.7/checkin-food-seaview.js'});

let html=previousHtml.replaceAll(PREVIOUS,VERSION);
...
html=mustReplace(html,'</head>',`<style id="checkin-food-seaview-v257">${addonCss}</style>\n</head>`,'v2.5.7 stylesheet injection');
...
html=mustReplace(html,'</body>',`${dataScript}\n${addonScript}\n</body>`,'v2.5.7 runtime injection');

const mustHave=['琴屿路','海之恋Park','宫崎骏漫画墙','龙江路','黄岛鱼鸣嘴','黄岛站路牌','小鱼山','黄县路与黄县支路交汇口','福山支路','燕儿岛山公园','OHMO CAFE','五仁杂货铺','龙口路 → 龙江路 → 黄县路','福山支路 → 金口一路','双合园','万和春','海平馄饨','宽哥烧烤海鲜大排档','hey妞酸奶','SOJU笑猪食堂','一抹崂山绿','班布大叔的店','WUYOO韩国冰奶冰茶咖啡','味乐通炸鸡','老西镇臭豆腐','石雀滩','唐岛湾','青岛海神庙','即墨区滨海公园','月牙湾海滩'];
for(const token of mustHave)if(!html.includes(token))throw new Error(`required v2.5.7 content missing: ${token}`);
if(!html.includes('TravelCheckinSpots'))throw new Error('check-in runtime missing');
if(!html.includes('data-tab=\'checkin\'')&&!html.includes('data-tab="checkin"'))throw new Error('check-in tab runtime token missing');
if(!html.includes('/v5/place/text'))throw new Error('AMap photo query integration missing');

[...html.matchAll(/<script(?![^>]*\\bsrc=)[^>]*>([\\s\\S]*?)<\\/script>/gi)].forEach((match,index)=>new vm.Script(match[1],{filename:`v257-inline-${index}.js`}));
...
const initialBudget=Number(previousManifest.gzipBytes)+75_000;
const totalBudget=Number(previousManifest.totalGzipBytes)+82_000;
...
const assetDir=path.join(ROOT,'assets',`v${VERSION}`);fs.mkdirSync(assetDir,{recursive:true});for(const name of fs.readdirSync(assetDir))fs.rmSync(path.join(assetDir,name),{recursive:true,force:true});
const chunkSize=Math.ceil(gzip.length/4);for(let index=0;index<4;index+=1)fs.writeFileSync(path.join(assetDir,`payload-${index}.b64`),gzip.subarray(index*chunkSize,Math.min(gzip.length,(index+1)*chunkSize)).toString('base64'));
write(`assets/v${VERSION}/lazy-tools.js`,lazySource);
const manifest={version:VERSION,previous:PREVIOUS,rollback:ROLLBACK,fallback:STABLE,htmlBytes:Buffer.byteLength(html),gzipBytes:gzip.length,lazyBytes:Buffer.byteLength(lazySource),lazyGzipBytes:lazyGzip.length,totalGzipBytes:gzip.length+lazyGzip.length,initialDeltaBytes:gzip.length-Number(previousManifest.gzipBytes),totalDeltaBytes:gzip.length+lazyGzip.length-Number(previousManifest.totalGzipBytes),sha256:sha(html),lazySha256:sha(lazySource),builtAt:RELEASE_TIMESTAMP,baseVersion:PREVIOUS,baseSha256:previousManifest.sha256,frozenRollbackVersion:ROLLBACK,frozenRollbackSha256:FROZEN_V254_HASH,capabilities:[...(previousManifest.capabilities||[]),'checkin-camera-spots','multi-camera-per-place','photo-route-layer','amap-poi-photo-gallery','expanded-food-wishlist','bbq-reference-pool','sea-view-beach-play']};
...
versionIndex=versionIndex.replace(new RegExp(`<div class="card(?: current)?"><b><a href="${DATE}-v${VERSION}\\.html">[\\s\\S]*?<\\/p><\\/div>`,'g'),'');
versionIndex=versionIndex.replaceAll('class="card current"','class="card"');
const card=`<div class="card current"><b><a href="${DATE}-v${VERSION}.html">v${VERSION} 打卡机位与美食海景扩充版</a></b><div class="tag">当前线上版本</div><p>新增独立打卡机位标签页、多机位地图、两条重点拍照路线、高德地点照片、逐日打卡支线、必吃必喝/烤肉参考池、海景与沙滩候选；v2.5.4冻结回退保持不变。</p></div>`;
versionIndex=versionIndex.replace(/(<body><h1>青岛旅行地图历史版本<\\/h1>)/,`$1${card}`);write('versions/index.html',versionIndex);
