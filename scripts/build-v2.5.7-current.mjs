...
if (sha(previousHtml) !== previousManifest.sha256)
  throw new Error(`v${PREVIOUS} source does not match its manifest`);
if (previousManifest.frozenRollbackSha256 !== FROZEN_V254_HASH)
  throw new Error('v2.5.6 frozen rollback hash drifted');
...
html = mustReplace(
  html,
  '</head>',
  `<style id="checkin-food-seaview-v257">${addonCss}</style>\n</head>`,
  'v2.5.7 stylesheet injection',
);
...
for (const token of mustHave)
  if (!html.includes(token)) throw new Error(`required v2.5.7 content missing: ${token}`);
if (!html.includes('TravelCheckinSpots')) throw new Error('check-in runtime missing');
if (!html.includes("data-tab='checkin'") && !html.includes('data-tab="checkin"'))
  throw new Error('check-in tab runtime token missing');
...
[...html.matchAll(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/gi)].forEach(
  (match, index) => new vm.Script(match[1], { filename: `v257-inline-${index}.js` }),
);
...
if (gzip.length > initialBudget)
  throw new Error(`Initial gzip budget exceeded: ${gzip.length} > ${initialBudget}`);
...
for (let index = 0; index < 4; index += 1)
  fs.writeFileSync(
    ...
    gzip
      .subarray(index * chunkSize, Math.min(gzip.length, (index + 1) * chunkSize))
      .toString('base64'),
  );
...
const manifest = {
  ...
  capabilities: [
    ...
    'checkin-camera-spots',
    'multi-camera-per-place',
    'photo-route-layer',
    'amap-poi-photo-gallery',
    'expanded-food-wishlist',
    'bbq-reference-pool',
    'sea-view-beach-play',
  ],
};
...
write(
  ...
  `${JSON.stringify({ version: VERSION, previous: PREVIOUS, previousInitialGzip: Number(previousManifest.gzipBytes), previousTotalGzip: Number(previousManifest.totalGzipBytes), initialGzip: gzip.length, lazyGzip: lazyGzip.length, totalGzip: gzip.length + lazyGzip.length, initialBudget, totalBudget, passed: true }, null, 2)}\n`,
);
...
function loader(pinned = false) {
  const candidates = pinned
    ? `['${VERSION}']`
    : `['${VERSION}','${PREVIOUS}','${PREVIOUS2}','${ROLLBACK}','${STABLE}']`;
...
let versionIndex = fs.existsSync(path.join(ROOT, 'versions/index.html'))
  ? read('versions/index.html')
  : '<!doctype html><html lang="zh-CN"><body><h1>青岛旅行地图历史版本</h1></body></html>';
versionIndex = versionIndex.replace(
  new RegExp(
    `<div class="card(?: current)?"><b><a href="${DATE}-v${VERSION}\\.html">[\s\S]*?<\\/p><\\/div>`,
    'g',
  ),
  '',
);
...
const currentPayloads = [0, 1, 2, 3]
  .map((index) => `'./assets/v${VERSION}/payload-${index}.b64'`)
  .join(',');
const previousPayloads = [0, 1, 2, 3]
  .map((index) => `'./assets/v${PREVIOUS}/payload-${index}.b64'`)
  .join(',');
