...
v3 采用旁路架构，让未来 Web 和小程序共享青岛领域模型、运行时 Schema、Planner 契约和版本迁移，同时保持 v2 构建链独立可回退。当前 Draft PR 已完成 Phase 0–3 的 sidecar，并建立 Phase 4 候选内容闭环：49 个现有点位分类、17 个日程模块、17 套可编辑预设、来源／推广风险、季节信息、更新任务、人工审核状态机以及发布／回滚门禁。v2.5.4 的 24 项来源、8 项预约、3 家候选酒店、17 项必去、10 个愿望地图点和 12 项必吃必买已通过只读快照逐项迁入候选目录；另有 3 个医院锚点和药店／厕所／停车／充电 4 类运行时服务候选。所有新增内容仍为 `review-required`，不能视为正式攻略发布。v3 尚未替换正式入口；正式地图、真实路线、动态 Provider、逐日完整时间轴与攻略正文、小程序仍受后续门禁约束。
