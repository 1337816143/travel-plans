...
当前质量门禁结果：上述七类检查均通过。Playwright 已覆盖 1366×768 桌面视口和 430×932 Android 竖屏视口；测试确认无页面横向溢出、手机顶部控制不重叠、路线总览位于可视区域内、侧栏能够正常开合，实时路况开关不改变地图中心和缩放。
...
5. 将剩余覆盖物接入命名图层；
6. 完成用户预览确认后，再决定是否将 v2 合并到 `main`。
