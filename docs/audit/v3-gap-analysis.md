...
本 Draft PR 已在旁路完成 Planner、Phase 3 编辑器和 Phase 4 内容候选工程闭环，并通过生成快照无损迁入 24 项来源、8 项预约、3 家酒店、17 项必去、10 个愿望地图点和 12 项必吃必买；这些候选仍没有人工审核签名，不能伪装成正式攻略。Legacy 的逐日完整时间轴、攻略正文、Plan B、用户状态与全部旅行工具仍未达到替换门禁。项目仍不生成正式图标资产、不选定小程序框架、不修改高德凭据、不改 Service Worker、不切换 Pages、不删除 v2.5.4／v1.0.15，也不自动合并。当前路线和住宿时间仍是明确标记的直线降级估算；正式 Provider 网关、人工内容审批、剩余 Legacy 功能逐项迁移和小程序 PoC 必须继续按 Phase 4–6 门禁推进。详见 `phase4-content-gap-report.md`。
