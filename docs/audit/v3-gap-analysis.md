...
v2.5.4 已从早期单文件逐步拆出数据、地图、服务、状态和 UI 模块，但运行时仍以 Legacy 全局变量和 DOM 协作为中心。它适合继续作为稳定产品与迁移样本，不适合作为自由规划器、多个计划、共享小程序核心的直接宿主。v3 因此采用“双工作区”：先在“完整攻略”中精确运行冻结的 v2.5.4，保证用户不因重构失去任何功能；再在“自定义规划”中使用纯 TypeScript 领域核心和平台地图 adapter 增加能力。原功能在新实现达到替换门禁前不得从完整攻略中删除。
...
v3 已恢复用户侧完整性：默认工作区直接加载哈希与 v2.5.4 manifest 一致的完整页面；自定义规划器也已接入真实 Leaflet 多底图、缩放、拖动、定位、底图容灾、点位弹窗和高德坐标导航。`check-v3-v2-parity.mjs` 会同时核对完整页面哈希、49／8／8／3／24／17／10／12 数据计数、原版关键功能入口及 v3 真实地图 surface，防止再次把删减版当成完成。

这不等于每项 Legacy 实现已经原生重写。逐日攻略正文、Plan B、预约状态、全部旅行工具、高德天气／路况／真实道路路线目前仍由“完整攻略”中的原版 v2.5.4 执行；它们在新 adapter 达到等价回归前不可删除。候选内容也仍没有人工审核签名。当前自定义路线和住宿距离继续明确标记为直线降级；正式 Provider 网关、人工内容审批、深层状态桥接和小程序 PoC 仍须按门禁推进。详见 `phase4-content-gap-report.md`。
